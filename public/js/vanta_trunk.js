import P5Base, {VANTA} from 'static/js/_p5Base.js'
import {color2Hex, mobileCheck} from 'static/js/helpers.js'
...
function display(){
  ...
  // Add a glow to the lines
  let glowColor = color2Hex(t.options.color).slice(0, 3) + '00';
  p.stroke(glowColor, 10);
  for(let i = 0; i < rings; i ++){
    p.beginShape()
    for(let angle = 0; angle < 360; angle++){
      let radian = p.radians(angle)
      let radius = (t.options.chaos * chaos_mag * getNoiseWithTime(radian, chaos_delta * i + chaos_init, oz))
        + (dim_delta * i + dim_init)
        + (i * t.options.spacing || 0)
      p.vertex(radius * p.cos(radian) + 0.1, radius * p.sin(radian) + 0.1)
    }
    p.endShape(p.CLOSE)
  }
}
...
function getNoiseWithTime (radian, dim, time){
  let r = radian % p.TWO_PI;
  if(r < 0.0){r += p.TWO_PI;}
  return p.noise(ox + p.cos(r) * dim, oy + p.sin(r) * dim, time);
}